-- Fix handle_new_user function to include applying_to field
-- This resolves the database constraint violation during signup

-- Drop the old trigger and function
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
DROP FUNCTION IF EXISTS public.handle_new_user();

-- Create the corrected trigger function that includes applying_to
...
BEGIN
  ...
  VALUES (
    ...
    NEW.email,
    COALESCE(NEW.raw_user_meta_data ->> 'applying_to', 'undergraduate')::TEXT,
    false
  );
...
COMMENT ON FUNCTION public.handle_new_user IS 'Creates user_profiles record when new user signs up. Includes applying_to field from metadata to avoid constraint violations.';
