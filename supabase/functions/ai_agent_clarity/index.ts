...
You must provide:
1. An overall quality_score from 1-10 rating how clear and concise the entire essay is
2. Multiple specific comments (2-4) focusing on clarity and conciseness
...
- Identify specific text that needs clarification
- Suggest how to make it clearer and more precise
- Be specific about positioning with text_selection

For scores < 8, format improvement suggestions as: "Instead of saying X, you could phrase it as Y to make it clearer and more direct."
...
RESPONSE FORMAT (JSON only - must be valid JSON):
{
  ...
  "comments": [
    {
      "comment_text": "[Specific suggestion for making the identified text clearer and more concise. Include quality score assessment and conditional feedback based on overall score. If overall score < 8, provide improvement suggestions. If overall score >= 8, focus on praise.]",
      "comment_nature": "weakness",
      "comment_category": "inline",
      "agent_type": "clarity",
      "text_selection": {
    ...
    {
      "comment_text": "[Another specific clarity suggestion with precise positioning. Include quality score assessment and conditional feedback based on overall score.]",
      "comment_nature": "weakness", 
      "comment_category": "inline",
      "agent_type": "clarity",
      "text_selection": {
...
CRITICAL: Your response must be valid JSON only. Do not include any text before or after the JSON object. Ensure all strings are properly escaped and all brackets are balanced. Focus ONLY on clarity, conciseness, and precision. Do NOT address grammar, spelling, punctuation, content, or style.`
  ...
  try {
    ...
    try {
      ...
      if (jsonStart === -1 || jsonEnd === -1 || jsonEnd <= jsonStart) {
        console.error(`No valid JSON boundaries found in clarity agent response:`, responseText)
      ...
      console.error(`JSON parsing error in clarity agent:`, parseError.message)
      ...
      try {
        ...
        throw new Error(`Failed to parse JSON from clarity agent response: ${parseError.message}. Additional error: ${arrayError.message}`)
      }
    }
