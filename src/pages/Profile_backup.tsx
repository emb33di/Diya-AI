...
export default function Profile() {
    ...
    try {
      ...
      const formData: any = { ...combinedProfile };
        ...
        const sanitizedFormData = {
          ...
          // Financial Information
          looking_for_scholarships: formData.looking_for_scholarships ?? undefined,
          looking_for_financial_aid: formData.looking_for_financial_aid ?? undefined,
  ...
  // Test Enhanced Profile Extraction Function
  const testEnhancedProfileExtraction = async () => {
    try {
      ...
      const transcript = conversations[0].transcript;
      
      console.log('🧪 TESTING ENHANCED PROFILE EXTRACTION:');
      console.log('📞 Conversation ID:', conversationId);
      console.log('📄 Transcript Length:', transcript?.length || 0, 'characters');
      console.log('📅 Conversation Date:', conversations[0].created_at);
      console.log('👤 User ID:', user.id);
      
      // Show first 200 characters of transcript for debugging
      if (transcript) {
      ...
      if (response.success && response.data) {
        ...
        if (extractedProfile.personal_info) {
          ...
          // Map citizenship status to country code if available
          if (extractedProfile.personal_info.citizenship_status && !formData.country_code) {
            if (extractedProfile.personal_info.citizenship_status === 'International Student') {
              formData.country_code = 'IN'; // Default to India for international students
              newAIPopulatedFields.add('country_code');
            }
          }
        }
        ...
        // Program-specific career mapping
        if (schoolType === 'MBA') {
          // MBA-specific career mapping
          if (extractedProfile.professional_experience) {
            const careerParts = [];
            if (extractedProfile.professional_experience.current_role) {
              careerParts.push(extractedProfile.professional_experience.current_role);
            }
            if (extractedProfile.professional_experience.industry) {
              careerParts.push(extractedProfile.professional_experience.industry);
            }
            // Map work experience summary to career interests
            if (extractedProfile.professional_experience.work_experience_summary) {
              careerParts.push(...extractedProfile.professional_experience.work_experience_summary);
            }
            if (careerParts.length > 0) {
              // Combine with existing career interests if any
              const existingCareer = formData.career_interests || '';
              formData.career_interests = existingCareer ? 
                `${existingCareer}, ${careerParts.join(', ')}` : 
                careerParts.join(', ');
              newAIPopulatedFields.add('career_interests');
            }
          }
          
          // Map MBA career goals
          if (extractedProfile.career_goals) {
            const careerGoals = [];
            if (extractedProfile.career_goals.post_mba_goals) {
              careerGoals.push(...extractedProfile.career_goals.post_mba_goals);
            }
            if (extractedProfile.career_goals.target_industry) {
              careerGoals.push(...extractedProfile.career_goals.target_industry);
            }
            if (extractedProfile.career_goals.target_function) {
              careerGoals.push(...extractedProfile.career_goals.target_function);
            }
            if (extractedProfile.career_goals.career_switch_goals) {
              careerGoals.push(...extractedProfile.career_goals.career_switch_goals);
            }
            if (careerGoals.length > 0) {
              // Combine with existing career interests if any
              const existingCareer = formData.career_interests || '';
              formData.career_interests = existingCareer ? 
                `${existingCareer}, ${careerGoals.join(', ')}` : 
                careerGoals.join(', ');
              newAIPopulatedFields.add('career_interests');
            }
          }
        } else if (schoolType === 'LLM') {
          // LLM-specific career mapping
          if (extractedProfile.career_goals) {
            const careerGoals = [];
            if (extractedProfile.career_goals.law_specialization) {
              careerGoals.push(...extractedProfile.career_goals.law_specialization);
            }
            if (extractedProfile.career_goals.target_practice_area) {
              careerGoals.push(...extractedProfile.career_goals.target_practice_area);
            }
            if (careerGoals.length > 0) {
              formData.career_interests = careerGoals.join(', ');
              newAIPopulatedFields.add('career_interests');
            }
          }
        } else if (schoolType === 'PhD') {
          // PhD-specific career mapping
          if (extractedProfile.career_goals) {
            const careerGoals = [];
            if (extractedProfile.career_goals.research_interests) {
              careerGoals.push(...extractedProfile.career_goals.research_interests);
            }
            if (extractedProfile.career_goals.academic_goals) {
              careerGoals.push(...extractedProfile.career_goals.academic_goals);
            }
            if (careerGoals.length > 0) {
              formData.career_interests = careerGoals.join(', ');
              newAIPopulatedFields.add('career_interests');
        ...
        if (extractedProfile.preferences) {
          ...
          // Map program type to preferences
          if (extractedProfile.preferences.program_type) {
            // This could be mapped to a program preference field if it exists
            console.log('📋 Program Type Preference:', extractedProfile.preferences.program_type);
          }
          // Map geographic preferences
          if (extractedProfile.preferences.geographic_preference) {
            console.log('🌍 Geographic Preferences:', extractedProfile.preferences.geographic_preference);
          }
          // Map school preferences
          if (extractedProfile.preferences.school_preferences) {
            console.log('🏫 School Preferences:', extractedProfile.preferences.school_preferences);
          }
        }

        // Financial considerations
        if (extractedProfile.financial_considerations) {
          // Map funding sources to looking_for_scholarships
          if (extractedProfile.financial_considerations.funding_sources) {
            console.log('💰 Funding Sources:', extractedProfile.financial_considerations.funding_sources);
            // If seeking scholarships, mark as yes
            if (extractedProfile.financial_considerations.funding_sources.includes('Scholarships')) {
              formData.looking_for_scholarships = 'yes';
          ...
          if (extractedProfile.financial_considerations.financial_aid_importance) {
            if (extractedProfile.financial_considerations.financial_aid_importance !== 'Not a factor') {
              ...
              formData.looking_for_financial_aid = 'no';
              newAIPopulatedFields.add('looking_for_financial_aid');
            }
          }
        }
        ...
        if (extractedProfile.extracurricular_activities) {
          if (extractedProfile.extracurricular_activities.activities) {
            console.log('🎭 Extracurricular Activities:', extractedProfile.extracurricular_activities.activities);
          }
          if (extractedProfile.extracurricular_activities.leadership_roles) {
            console.log('👑 Leadership Roles:', extractedProfile.extracurricular_activities.leadership_roles);
          }
          if (extractedProfile.extracurricular_activities.personal_projects) {
            console.log('🔧 Personal Projects:', extractedProfile.extracurricular_activities.personal_projects);
          }
        }

        // Additional info
        if (extractedProfile.additional_info) {
          if (extractedProfile.additional_info.application_concerns) {
            console.log('⚠️ Application Concerns:', extractedProfile.additional_info.application_concerns);
          }
          if (extractedProfile.additional_info.specific_questions) {
            console.log('❓ Specific Questions:', extractedProfile.additional_info.specific_questions);
          }
        }
        ...
        // Detailed debugging for auto-populated fields
        console.log('🔍 DETAILED FIELD MAPPING DEBUG:');
        console.log('📋 Raw Extracted Profile:', extractedProfile);
        console.log('📝 Form Data Mapped:', formData);
        console.log('✅ Fields Auto-Populated:', Array.from(newAIPopulatedFields));
        
        // Debug the structure of extracted profile
        console.log('🔍 EXTRACTED PROFILE STRUCTURE ANALYSIS:');
        if (extractedProfile.personal_info) {
          console.log('👤 Personal Info:', extractedProfile.personal_info);
        }
        if (extractedProfile.academic_background) {
          console.log('🎓 Academic Background:', extractedProfile.academic_background);
        }
        if (extractedProfile.professional_experience) {
          console.log('💼 Professional Experience:', extractedProfile.professional_experience);
        }
        if (extractedProfile.test_scores) {
          console.log('📊 Test Scores:', extractedProfile.test_scores);
        }
        if (extractedProfile.career_goals) {
          console.log('🎯 Career Goals:', extractedProfile.career_goals);
        }
        if (extractedProfile.preferences) {
          console.log('⚙️ Preferences:', extractedProfile.preferences);
        }
        if (extractedProfile.financial_considerations) {
          console.log('💰 Financial Considerations:', extractedProfile.financial_considerations);
        }
        
        // Show field-by-field mapping
        console.log('🎯 FIELD-BY-FIELD MAPPING:');
        ...
        const allPossibleFields = [
          ...
          'sat_score', 'act_score', 'career_interests', 'ideal_college_size', 'ideal_college_setting',
          'must_haves', 'deal_breakers', 'looking_for_scholarships', 'looking_for_financial_aid'
        ];
        const notPopulatedFields = allPossibleFields.filter(field => !newAIPopulatedFields.has(field));
        console.log('❌ Fields NOT Auto-Populated:', notPopulatedFields);

        toast({
          title: "🧪 Profile Extraction Test Complete",
          description: `Successfully extracted profile with ${profileData.confidence_score}% confidence. ${newAIPopulatedFields.size} fields auto-filled!`,
        });

        console.log('📊 Extraction Results Summary:', {
          conversation_id: conversationId,
          confidence_score: profileData.confidence_score,
          fields_extracted: profileData.fields_extracted?.length,
          fields_missing: profileData.fields_missing?.length,
          auto_filled_fields: newAIPopulatedFields.size,
          auto_filled_field_names: Array.from(newAIPopulatedFields),
          not_populated_fields: notPopulatedFields,
        ...
        toast({
          ...
          description: response.error || "Failed to extract profile information.",
          variant: "destructive",
        });
      }
      ...
      toast({
        title: "Test Error",
        description: "Failed to test profile extraction. Please try again.",
        variant: "destructive",
      });
    } finally {
        ...
        <form key={profileData ? 'loaded' : 'loading'} onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <div className="flex justify-end gap-4">
            ...
            <Button 
              ...
              onClick={testEnhancedProfileExtraction}
